...
cpu_lu_xeon <- cpu_lu_xeon |> select(Function, threads, gpus, nb, n, time, gflops, setup)

cpu_ch_xeon <- read.csv(paste0('experiments/csv/', latest_files['cpu_ch_xeon'], '.csv'))
cpu_ch_xeon$setup = 'cpu only'
cpu_ch_xeon <- cpu_ch_xeon |> select(Function, threads, gpus, nb, n, time, gflops, setup)
...
cpu_lu_i9 <- cpu_lu_i9 |> select(Function, threads, gpus, nb, n, time, gflops, setup)

cpu_ch_i9 <- read.csv(paste0('experiments/csv/', latest_files['cpu_ch_i9'], '.csv'))
cpu_ch_i9$setup = 'cpu only'
cpu_ch_i9 <- cpu_ch_i9 |> select(Function, threads, gpus, nb, n, time, gflops, setup)
...
cpu_i9 |>
  ...
  mutate(config = case_when(
           ...
           threads == 24 ~ "8 P-cores + 16 E-cores + 0 CUDA",
           threads == 31 ~ "7 P-cores + HT + 16 E-cores + 1 CUDA",
           threads == 32 ~ "8 P-cores + HT + 16 E-cores + 0 CUDA"),
         HT = case_when(
           threads == 7 ~ 0,
           threads == 8 ~ 0,
           threads == 15 ~ 1,
...
gpu_lu_i9 <- gpu_lu_i9 |> select(Function, threads, gpus, nb, n, time, gflops, setup)

gpu_ch_i9 <- read.csv(paste0('experiments/csv/', latest_files['gpu_ch_i9'], '.csv'))
gpu_ch_i9$setup = 'cpu+gpu'
gpu_ch_i9 <- gpu_ch_i9 |> select(Function, threads, gpus, nb, n, time, gflops, setup)
...
gpu_lu_ecore <- gpu_lu_ecore |> select(Function, threads, gpus, nb, n, time, gflops, setup)

gpu_ch_ecore <- read.csv(paste0('experiments/csv/', latest_files['gpu_ch_ecore'], '.csv'))
gpu_ch_ecore$setup = 'cpu+gpu'
gpu_ch_ecore <- gpu_ch_ecore |> select(Function, threads, gpus, nb, n, time, gflops, setup)
